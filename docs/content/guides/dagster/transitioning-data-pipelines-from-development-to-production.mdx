...
Now we can add these assets to our <PyObject object="Definitions" /> object and materialize them via Dagit as part of our local development workflow. We will use the [configured API](/concepts/configuration/configured) to add configuration for the `snowflake_pandas_io_manager`.
