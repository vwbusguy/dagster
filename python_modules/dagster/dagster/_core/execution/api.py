        ...
        if pipeline_run.status not in (DagsterRunStatus.NOT_STARTED, DagsterRunStatus.STARTING):
            if instance.run_monitoring_enabled:
                ...
                def gen_ignore_duplicate_run_worker():
                    yield instance.report_engine_event(
                        "Ignoring a duplicate run that was started from somewhere other than the"
                        " run monitor daemon",
                        pipeline_run,
                    )
                ...
                def gen_fail_restarted_run_worker():
                    yield instance.report_engine_event(
                        f"{pipeline_run.pipeline_name} ({pipeline_run.run_id}) started a new run"
                        f" worker while the run was already in state {pipeline_run.status}. This"
                        " most frequently happens when the run worker unexpectedly stops and is"
    ...
    try:
        ...
        if pipeline_canceled_info:
            ...
            if reloaded_run and reloaded_run.status == DagsterRunStatus.CANCELING:
                ...
                event = DagsterEvent.engine_event(
                    pipeline_context,
                    "Execution was interrupted unexpectedly. No user initiated termination request"
                    " was found, not treating as failure because run will be resumed.",
                    EngineEventData(),
                )
            else:
